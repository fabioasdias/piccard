...
if __name__ == '__main__':
    ...
    for city in cities['features']:
        ...
        if (kind=='SY'):
            ...
            makeBuff=makeBuff+"\tzip -9j {0}/normGeoJsons.zip {0}/*_n.gj\n".format(folder)
            makeBuff=makeBuff+"\trm {0}/*_n.gj\n".format(folder)
        makeBuff=makeBuff+"{0}/display.json: {0}/normGeoJsons.zip {0}/basegraph.gp prepGJs.py\n".format(folder)
        makeBuff=makeBuff+"\tpython3 prepGJs.py {0}/normGeoJsons.zip {0}/basegraph.gp {0}/display.json\n".format(folder)        
