...
def _nhgis(folder):
    codebooks = [x for x in glob(
        join(folder, '*.txt')) if 'codebook' in basename(x)]
...
def _infer_aspects_nhgis(df):
    ...
    prefixes = list(set([x[:3] for x in cols if (
        (len(x) == 6) and all([y.isdigit() for y in x[4:]]))]))
    prefixes = prefixes + \
        list(set([x[:4] for x in cols if ((len(x) == 7)
                                          and all([y.isdigit() for y in x[5:]]))]))
    for p in prefixes:
...
def _process_nhgis(nh, uploadDir, remoteIP):
    ...
    for i in range(len(nh)):
        try:
            ...
            cdf = pd.read_csv(csv, low_memory=False, encoding='latin1', dtype={
                              'GISJOIN': object}, index_col=False)
            cdf = cdf.set_index('GISJOIN').dropna(
                axis=1, how='all')  # drops columns that are empty
            ...
            if year not in descriptions:
                ...
                df[year] = pd.merge(
                    cdf, df[year], on='GISJOIN', suffixes=['', '_x'])
    ...
    for year in df:
        ...
        info = {'id': id,
                'year': year,
                # check in conf.json
                'geometry': {'name': 'US_CT_{0}'.format(year)},
                'ip': str(remoteIP),
                'index': 'GISJOIN',
        ...
        for c in df[year].columns:
            info['samples'][c] = str(
                df[year][c][df[year][c].first_valid_index()])
...
def processUploadFolder(tempDir, uploadDir, remoteIP):
    ...
    print('found: csvs {0}, jsons {1}, nhgis {2}'.format(
        len(csvs), len(jsons), len(nh)))
    print(csvs)
    print(jsons)
    print(nh)
